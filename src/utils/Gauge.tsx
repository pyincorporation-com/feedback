...
export class PatchedInterpolation<Input, Output> extends Interpolation<
  Input,
  Output
> {
...
Globals.assign({
  to: (source, args) => new PatchedInterpolation(source, args),
});
...
const warn = (condition: boolean, message: string) =>
  !condition && console.warn(message);
...
export const Gauge = forwardRef<SVGSVGElement, GaugeProps>(
  (
    ...
    warn(
      startValue >= minValue && startValue <= maxValue,
      "startValue should be between minValue and maxValue"
    );
    warn(
      0 <= startAngleDeg && startAngleDeg < 360,
      "startAngle should be at least 0 and less than 360"
    );
    warn(
      0 <= endAngleDeg && endAngleDeg < 360,
      "endAngle should be at least 0 and less than 360"
    );
    warn(roundDigits >= 0, "roundDigits should be nonnegative");
    warn(
      0 <= arcWidthFactor && arcWidthFactor <= 2,
      "arcWidth should be at least 0 and at most 2"
    );
    warn(
      0 <= trackWidthFactor && trackWidthFactor <= 2,
      "trackWidth should be at least 0 and at most 2"
    );
    warn(
  ...
  */
    ...
    const radius =
      ...
      (2 + Math.max(arcWidthFactor, trackWidthFactor));
    ...
    const trackCornerRadius =
      radius * trackWidthFactor * trackCornerRadiusFactor;
  ...
  */
    ...
    const renderNode = (renderable: RenderableNode) =>
      typeof renderable === "function"
        ? renderable({
          ...
          normValue: spring.value.to((value) =>
            inverseLerp(minValue, maxValue, value)
          ),
          rawValue: rawValue,
        })
        : renderable;
    ...
    return (
      <svg
        ...
        {...rest}
        viewBox={`${-containerRadius} ${-containerRadius} ${containerRadius * 2
          } ${containerRadius * 2}`}
        style={{
          width: "100%",
          height: "100%",
